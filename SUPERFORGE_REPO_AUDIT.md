...
| PM Tracking | Preventive maintenance behavioral reference | Rebuild PM natively inside MFGForge/SuperForge instead of copying the old app as source. |
...
MFGForge is the current SuperForge host application. It should remain the ERP source repo for core Flask code, SQLite schema, module routing, shared navigation, reporting, intelligence governance, native PM rebuild work, and future packaging scripts.
...
- Existing table families include master data, production, quality, documents/certificates, maintenance, quoting, supplier/planning, performance/pulse, governance, and `system_meta`.
...
- More module-specific tests should be added before importing ForgeQC, ForgeVault, or implementing native PM behavior.
...
- Native PM rebuild target anchored to MFGForge identity tables and future `modules/pm`.
...
- `pm_assets` overlaps with the old PM Tracking behavior, but PM will be rebuilt natively instead of source-migrated.

### Known risks

- Do not blindly copy external repos into MFGForge.
- Do not commit private records, customer drawings, local databases, Excel trackers, RMA data, cert files, PM databases, PM spreadsheets, machine history, or runtime exports.
...
ForgeQC should be treated as the source candidate for specialized quality workflows. Expected reusable areas include RMA tracking, NCR, DMR, CAPA, deviation workflow, customer/part/work-order quality history, reason-code and defect trend reporting, containment, root cause, corrective action, owner, due-date and closure lifecycle, quality metrics feeding reporting and Company Pulse, and quote/BOM quality review logic if present.
...
ForgeVault should be treated as the source candidate for controlled vendor, purchased part, material, supplier, document, and traceability logic. Expected reusable areas include vendor part intake, supplier records, approved purchased part records, controlled material records, document metadata and revision handling, McMaster-style vendor data intake patterns, and material cert workflows.
...
## PM Tracking reference position

PM Tracking is a behavioral reference, not a source migration target. PM will be rebuilt natively inside MFGForge/SuperForge.

Known reference details to preserve:
...
- Known behavior: dark dashboard UI, machine status cards, machine detail pages, manual operator completion entry, QR Codes page with printable SVG QR labels, Excel export at `/export.xlsx`, and planned maintenance ticket form.

Native PM rebuild targets:
...
- Company Pulse and planning risk signals

Do not commit `pm_data.xlsx`, `pm_app.db`, machine history, employee-level completion data, exported spreadsheets, QR labels with private URLs, or private shop records.

## SuperForge_Unofficial audit position

`SuperForge_Unofficial` should not become the application source of truth. Its proper role is compiled libraries, Windows executable release candidates, installer experiments, generated manifests, checksums, sanitized build logs, and packaging notes.
