...
MODULES: list[dict[str, Any]] = [
    ...
    {'area':'Master Data','key':'suppliers','title':'Suppliers','table':'suppliers','description':'Supplier master records for purchasing, outside processing, approved material sourcing, and quality follow-up.','fields':[field('name','Supplier name',True),field('code','Supplier code'),field('contact_name','Contact name'),field('email','Email',False,'email'),field('phone','Phone'),field('notes','Notes',False,'textarea')]},
    {'area':'Master Data','key':'departments','title':'Departments','table':'departments','description':'Shop departments used for scheduling, PM, FPY, quality trends, and aggregate morale indicators.','fields':[field('name','Department name',True),field('manager_name','Manager name'),field('notes','Notes',False,'textarea')]},
    ...
    {'area':'Quality','key':'deviations','title':'Deviation Requests','table':'deviations','description':'Controlled deviation requests with risk assessment and approval status.','fields':[field('deviation_number','Deviation number',True),field('requested_by','Requested by'),field('reason','Reason',True,'textarea'),field('proposed_disposition','Proposed disposition',False,'textarea'),field('risk_assessment','Risk assessment',False,'textarea')]},
...
def register_routes(app: Flask) -> None:
    ...
    def dashboard() -> str:
        ...
        body = render_template_string("""<section class='hero'><p class='eyebrow'>QCDSM ERP Foundation</p><h2>Manufacturing command center</h2><p>MFGForge connects quality, quoting, planning, purchasing, production, maintenance, documents, and morale signals into one controlled manufacturing system.</p></section><section class='qgrid'><article class='qcard'><span>Q</span><strong>Quality</strong><p>RMA, NCR, DMR, CAPA, FPY, deviations, and reason-code trends.</p></article><article class='qcard'><span>C</span><strong>Cost</strong><p>Materials, quote drafts, throughput baselines, and supplier cost signals.</p></article><article class='qcard'><span>D</span><strong>Delivery</strong><p>Work orders, planning watchlists, lead-time risk, and purchasing constraints.</p></article><article class='qcard'><span>S</span><strong>Safety</strong><p>Controlled records, review gates, traceability, and audit-ready approvals.</p></article><article class='qcard'><span>M</span><strong>Morale</strong><p>Aggregate department strain indicators tied to operational risk.</p></article></section>{% for area in areas %}<h2 class='area-title'>{{ area }}</h2><section class='grid'>{% for module in modules if module.area == area %}<a class='card' href='{{ url_for('list_records', module_key=module.key) }}'><small>{{ module.area }}</small><span class='count'>{{ counts[module.key] }}</span><h3>{{ module.title }}</h3><p>{{ module.description }}</p></a>{% endfor %}</section>{% endfor %}""", modules=MODULES, areas=AREAS, counts=counts)
